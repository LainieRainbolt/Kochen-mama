...
draw_text(
    ui_padding_x + (ui_border_size * 3) + 4,
    ui_padding_y + (ui_border_size * 10) + 4,
    "HP: " + string(global.player_hp) + "/" + string(global.player_max_hp)
);
draw_set(c_white, 1);
draw_text(
    ui_padding_x + (ui_border_size * 3),
    ui_padding_y + (ui_border_size * 10),
    "HP: " + string(global.player_hp) + "/" + string(global.player_max_hp)
);
...
for (var row = 0; row < inventory_rows; row++) {
    ...
    for (var column = 0; column < inventory_columns; column++) {
		...
		if(inventory_index <= array_length(inventory_items) - 1) {
			...
			draw_sprite(inventory_items[inventory_index].sprite, 0, pos_x + 50, pos_y+50);
		}
